...
async function renderStun() {
    ...
    $('#pageBody').innerHTML = `
      <div class="tip">
        ...
        需要外网设备主动连入时，必须保证路由器 WAN 口为公网并在任务中启用 UPnP（或在路由器手动做端口转发）。
        ...
        <b>但出站探测映射受 NAT 会话过滤限制，仅对已打洞的对端有效，外网设备主动连入大概率不可用</b>（此时自测成功仅说明映射存活），
        需向运营商申请公网 IP、使用对端打洞或中继；展示改用 STUN 出口公网 IP，也可在操作列点「自测」手动复测。
      </div>
